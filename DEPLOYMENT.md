...
Complete guide to deploy Smith's Lawn Care AI Assistant to your server.
...
# In PostgreSQL shell:
CREATE DATABASE smiths_lawn_care_crm;
CREATE USER hvacuser WITH ENCRYPTED PASSWORD 'your_secure_password';
GRANT ALL PRIVILEGES ON DATABASE smiths_lawn_care_crm TO hvacuser;
\q

# Import schema
sudo -u postgres psql -d smiths_lawn_care_crm -f /path/to/smiths-lawn-care-crm-schema.sql
```
...
git clone https://github.com/dutchiono/smiths-lawn-care.git .
...
BUSINESS_NAME=Your Smith's Lawn Care
...
```bash
# Create backup
pg_dump -U hvacuser smiths_lawn_care_crm > backup_$(date +%Y%m%d).sql

# Restore backup
psql -U hvacuser smiths_lawn_care_crm < backup_20240209.sql
```
...
psql -U hvacuser -d smiths_lawn_care_crm -c "SELECT 1"
...
# Test connection
psql -U hvacuser -d smiths_lawn_care_crm
```
...
# Backup database
pg_dump -U hvacuser smiths_lawn_care_crm > backup.sql
